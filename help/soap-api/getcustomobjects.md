...
description: Erfahren Sie, wie Sie mit getCustomObjects benutzerdefinierte Marketo-Objekte über SOAP mit Schlüsseln, includeAttributes, einer Batch-Größe von bis zu 100 und einer streamPosition-Paginierung abrufen können.
...
Ruft ein oder mehrere benutzerdefinierte Objekte mithilfe einer Kombination von Kriterien ab, die aus null oder einem benutzerdefinierten Objektschlüssel bestehen.

Gibt eine Liste übereinstimmender benutzerdefinierter Objekte zurück, alle von einem einzigen Typ, bis zu 100 in einem Batch, und ein [Stream-Position](stream-position.md)-Token zum Abrufen aufeinander folgender Batches.
...
| customObjKeyLists->keyList->attribute | Erforderlich | Das -Attribut ist ein Schlüssel/Wert-Paar, das zur Identifizierung der benutzerdefinierten Objekte verwendet wird, die Sie abrufen möchten. Sie können mehrere Attribute im `customObjKeyLists` angeben |
...
| streamPosition | Optional | Wird zum Paginieren durch mehrere Ergebnissätze verwendet. Der übergebene Wert ist der vom vorherigen `getCustomObjects`-Aufruf zurückgegebene Wert. |
